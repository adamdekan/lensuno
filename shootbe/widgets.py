...
class CountableWidget(widgets.Textarea):
    ...
    def render(self, name, value, attrs=None, **kwargs):
        ...
        if final_attrs.get("data-min-count") and not isinstance(
            final_attrs.get("data-min-count"), int
        ):
            final_attrs.pop("data-min-count")
        if final_attrs.get("data-max-count") and not isinstance(
    ...
    def get_word_count_template(attrs):
        ...
        return (
            '<div class="text-xs text-gray-500"><span class="text-count" id="%(id)s_counter">%(label)s'
            '<span class="text-count-current">%(number)s</span></span></div>\r\n'
        ) % {
            "label": count_label,
            "id": attrs.get("id"),
